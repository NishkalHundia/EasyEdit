#!/usr/bin/env python3
"""Apply STA vectors to generate text."""
...
def main():
    ...
    vector_dir = f"vectors/dialz/{args.model.split('/')[-1]}/{args.dataset}/sta_vector"
    
    if not os.path.exists(vector_dir):
        print(f"Error: Vectors not found at {vector_dir}")
        print(f"Generate them first with:")
        print(f"python dialz_generate_sta.py --dataset {args.dataset} --model {args.model} --layers {' '.join(map(str, args.layers))}")
        return
